...
class DBFParser extends EventEmitter
    ...
    _parseHead: (buffer) =>
        ...
        @fields = for i in [32 .. @headOffset - 32] by 32
            field = 
                name: (iconv.decode(buffer.slice(i, i+11), @decodeTo).toString().replace /[\u0000].*$/, '').replace /^[\r]+|[\n]+|[\r\n]+|[\n\r]+/, ''
    ...
    _parseField: (begin, end, field, buffer) =>
        switch field.type
            when 'C'
                value = iconv.decode(buffer.slice(begin, end), @decodeTo).toString().replace /^\x20+|\x20+$/g, ''
                if value
