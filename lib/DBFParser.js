...
(function() {
  ...
  DBFParser = (function(superClass) {
    ...
    DBFParser.prototype._parseHead = function(buffer) {
      ...
      this.fields = (function() {
        ...
        for (i = j = 32, ref = this.headOffset - 32; j <= ref; i = j += 32) {
          field = {
            name: (iconv.decode(buffer.slice(i, i + 11), this.decodeTo).toString().replace(/[\u0000].*$/, '')).replace(/^[\r]+|[\n]+|[\r\n]+|[\n\r]+/, ''),
    ...
    DBFParser.prototype._parseField = function(begin, end, field, buffer) {
      ...
      switch (field.type) {
        case 'C':
          value = iconv.decode(buffer.slice(begin, end), this.decodeTo).toString().replace(/^\x20+|\x20+$/g, '');
          if (value) {
